...
serve(async (req) => {
  ...
  try {
    ...
    // DISABLED: Message status updates (delivered, read) - removed to reduce Cloud consumption
    // Just return 200 OK immediately without processing
    if (event === 'messages.update' || event === 'MESSAGES_UPDATE' || event === 'messages_update') {
      console.log(`[SKIP] Ignoring message status update event: ${event}`);
      return new Response(JSON.stringify({ success: true, skipped: 'status_updates_disabled' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

    // DISABLED: Presence events (typing, recording) - removed to reduce Cloud consumption
    // Just return 200 OK immediately without processing
    if (event === 'presence' || event === 'PRESENCE') {
      console.log(`[SKIP] Ignoring presence event: ${event}`);
      return new Response(JSON.stringify({ success: true, skipped: 'presence_disabled' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
