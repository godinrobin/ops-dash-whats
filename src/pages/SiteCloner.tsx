              ...
              {currentResult && (
                <div className="grid gap-6 md:grid-cols-2">
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-3">
                      <div>
                        <span className="text-sm text-muted-foreground">Imagens ({currentResult.media.images.length}):</span>
                        {currentResult.media.images.length > 0 ? (
                          <div className="grid grid-cols-4 gap-2 mt-2">
                            {currentResult.media.images.slice(0, 8).map((img, i) => (
                              <div key={i} className="aspect-square bg-muted rounded overflow-hidden relative group">
                                <img 
                                  src={img.src} 
                                ...
                                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                  <a href={img.src} target="_blank" rel="noopener noreferrer" className="text-white text-xs">
                                    Ver
                                  </a>
                                </div>
                              </div>
                      ...
                      <div>
                        <span className="text-sm text-muted-foreground">Vídeos ({currentResult.media.videos.length}):</span>
                        {currentResult.media.videos.length > 0 ? (
                          <div className="flex flex-wrap gap-1 mt-1">
                            {currentResult.media.videos.map((video, i) => (
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-3">
                      <div>
                        <span className="text-sm text-muted-foreground">Títulos ({currentResult.content.headings.length}):</span>
                        {currentResult.content.headings.length > 0 ? (
                          <div className="max-h-24 overflow-y-auto mt-1">
                            {currentResult.content.headings.slice(0, 5).map((h, i) => (
                      ...
                      <div>
                        <span className="text-sm text-muted-foreground">Botões ({currentResult.content.buttons.length}):</span>
                        {currentResult.content.buttons.length > 0 ? (
                          <div className="flex flex-wrap gap-1 mt-1">
                            {currentResult.content.buttons.slice(0, 6).map((btn, i) => (
                              <Badge key={i} variant="secondary" className="text-xs">{btn}</Badge>
                            ))}
                          </div>
                        ) : (
                          <p className="text-sm text-muted-foreground">Nenhum botão encontrado</p>
                        )}
                      </div>
                      <div>
                        <span className="text-sm text-muted-foreground">Links: {currentResult.content.links.length}</span>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="md:col-span-2">
                    ...
                    <CardContent>
                      {currentResult.technical.technologies.length > 0 ? (
                        <div className="flex flex-wrap gap-2">
                          {currentResult.technical.technologies.map((tech, i) => (
                            <Badge key={i} variant="outline">{tech}</Badge>
                          ))}
                        </div>
                      ) : (
                        <p className="text-muted-foreground">Nenhuma tecnologia específica detectada</p>
                      )}
                    </CardContent>
                  </Card>
